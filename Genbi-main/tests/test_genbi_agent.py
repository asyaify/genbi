...
TEST_QUERIES = [
    ...
    {
        "query": "Топ-10 товаров по сумме реализации за 2024 год",
        "decomp": _make_decomp(
            metrics=["Реализация руб."],
            dimensions=["Наименование номенклатуры"],
            time_context="2024 год",
        ),
        "dax": "EVALUATE TOPN(10, SUMMARIZECOLUMNS('005 Номенклатура'[Наименование], FILTER(VALUES('000 Календарь'[Год]), '000 Календарь'[Год] = 2024), \"Реализация\", [Реализация руб.]), [Реализация руб.], DESC)",
        "chart_type": "bar",
    ...
    },
    {
        ...
        "dax": "EVALUATE SUMMARIZECOLUMNS('000 Календарь'[Месяц Номер], '000 Календарь'[Месяц], FILTER('000 Календарь', '000 Календарь'[Год] = 2024), \"Текущий год\", [Реализация руб.], \"Прошлый год\", [Реализация руб. (Прошлый год)]) ORDER BY '000 Календарь'[Месяц Номер] ASC",
        "chart_type": "line",
        "df_data": {
            "Месяц Номер": [1, 2, 3],
